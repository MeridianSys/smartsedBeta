...
function challengecollegue(){
  ...
  $userInfo = Configure::read('contoller')->get_user_info($id, 'Userprofile.title',false);
  if($userInfo['title']=='Dr.'){$cond = '0,2';}else{$cond = '0,1';}
  
           
  $listCat = $this->Quizcat->find('all',array('fields'=>'id','conditions'=>'access_type in ('.$cond.')'));
...
function challengetocolegue(){
     ...
     if($this->RequestHandler->isAjax()){
        ...
        if(!empty($user)){
            ...
            $checkfivechalenge = $this->Challenge->find('all',array('fields'=>'points','conditions'=>'points is NULL and setofquiznum IS NOT NULL and winning_status = 0 and from_id='.$this->Session->read('User.id')));
...
function challengedto(){
    ...
    //////////////// Chaeck //////////////// New User Challenge /////////////////
	    ...
	    if(count($countchallenge)>0){
	        for($i=0;$i<count($countchallenge);$i++){
	            ...
	            $newChallengeData['winning_status']=$countchallenge[$i]['Challengetemp']['winning_status'];
	            $this->Challenge->save($newChallengeData);
	            $this->Challengesenderattempt->query("update challenged_sender_attempts set challenged_id = ".$this->Challenge->id." where id IN (".$countchallenge[$i]['Challengetemp']['setofquiznum'].")");
    ...
    /////////////////////////END CHECKING //////////////////////////////////////
    $re = $this->Challenge->query("SELECT id , (TIME_TO_SEC(TIMEDIFF(NOW(),challenged_dt)))/3600 AS maintime FROM challenge_masters");
...
//SELECT TIME_TO_SEC(TIMEDIFF(NOW(),challenged_dt)) AS timest FROM `challenge_masters` WHERE id = 64
?>
