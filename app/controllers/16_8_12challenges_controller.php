...
class ChallengesController extends AppController {
	...
	var $name = 'Challenges';
        var $uses = array('User','Userprofile','Challengetemp','Challenge','Challengeuserattempt','Quizcat','Quizquestion','Quizanswer','Quizuserattempt');
...
function challengetocolegue(){
     ...
     if($this->RequestHandler->isAjax()){
        ...
        if(!empty($user)){
            ...
            if(count($result)<6){
                echo 'All question of this topic played by challenger';
            }else{
...
function  checkemail(){
     ...
     if($this->RequestHandler->isAjax()){
        ...
        if(!empty($user)){
            echo 'User exists in the Smartsed ';
        }else{
            echo 'User not exists in the Smartsed ';
        }
     }
}
...
function quizstart($id=NULL,$id2=NULl,$challengerid=NULL,$responcenotice=NULL){
	...
	$this->set('url',$id.'/'.$id2);
        ...
        $inarr = $this->Challengeuserattempt->find('all',array('fields'=>'qid','conditions'=>array('uid ='.$challengerid.' ')));
	$str='';
	if(count($inarr)>0){
	for($j=0;$j<count($inarr);$j++){

			$str = $str .','.$inarr[$j]['Challengeuserattempt']['qid'];
	...
	if(isset($this->data['Challenges']['qnum']) && !empty($this->data['Challenges']['qnum'])){
		...
		$cond=array('cat_id IN ('.$id.','.$id2.') and question_id  NOT IN  ('.$this->data['Challenges']['qid'].$str.')');

	}else{
		$qnum=1;
		if($str!=''){
		$cond=array('cat_id IN ('.$id.','.$id.') and question_id  NOT IN ('.substr($str,1).')');
		}else{
		$cond=array('cat_id IN ('.$id.','.$id.')');
		}

	}
	// For taking played qiuiz game //
	$this->Quizquestion->bindModel(array('hasOne'   => array(
	                                     'Quizuserattempt' => array('foreignKey' => false,'type'=>'inner','conditions'=>'Quizquestion.question_id=Quizuserattempt.qid and Quizuserattempt.uid = '.$challengerid))),
	                                false);
...
function quizexplaination($qid=NULL,$cid=NULL,$serialnum=NULL){
 ...
 // Check user athontication //
     ...
     // Retreve the Questions //
	    ...
	    //pr($this->Session->read('finalQuiz'));
...
* _______________________________________________________________________
*/
function viewplayedquizexplaination($qid=NULL,$cid=NULL,$serialnum=NULL,$useraid=NULL,$pd=NULL){
     // Check user athontication //
     $this->checkuserlogin();
     $inVal=explode('-',$cid);
     $records = $this->Quizcat->find('all',array('fields'=>'category_name','conditions'=>array('id IN ('.$inVal[0].','.$inVal[1].')')));
