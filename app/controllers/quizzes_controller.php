				...
				//$movecounter = 1;
                	while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                       	...
                       	if($catId!=0){ // Category exixts previously //
							$this->Quizcat->updateAll(array('access_type'=>$data[count($data)-1],array('id'=>$catId)));
                       		$tempQuestion['id']='';
							...
							$this->Quizquestion->save($tempQuestion);
                       		...
                       		for($j=2;$j<(count($data)-3);$j++){
                       			
                       			$tempans['id']='';
                         		$tempans['qid']=$tempQuestion['question_id'];
                         		$tempans['answer_text']=$data[$j];
                         		if($data[count($data)-3]==$ans){
                         		$tempans['answer_discription']=$data[count($data)-2];
                         		$tempans['result']='1';
                         		}else{
                       	 	...
                       	 	$tempcat['access_type']=$data[count($data)-1];
                       	 	$tempcat['parent_id']=0;
...
function deleteuploadedquiz($str=NULL,$id=NULL){
 ...
 function index(){
	   $this->layout = 'admin';
		...
		$this->checkadmin();
		$this->Quizcat->find('all');
		$this->paginate = array(
                         'limit' => 25,
                         'page'=>1,
                         'order'=>'Quizcat.id ASC'
    				 );
	     $data = $this->paginate('Quizcat');
         $this->set('data',$data);
}
...
function newcategory(){
	     ...
	     if(!empty($this->data)){
	     	 $contentArray['id']='';
                 $contentArray['category_name']=$this->data['Quizcat']['category_name'];
                 $contentArray['access_type']=$this->data['Quizcat']['accessType'];// 0:for All Student  1:for Non Medical 2:for Medical
                 $this->Quizcat->create();
                 if($this->Quizcat->save($contentArray)){
           	 $this->redirect('index');
                 }
              }
}
...
function editcategory($id=NULL){
 ...
 $accessList = array('0'=>'GEN','1'=>'SAT','2'=>'MED');
 $this->set('accessList',$accessList);
 $alldata = $this->Quizcat->find('all',array('conditions'=>'id='.$id));
    $this->set('data',$alldata);
       if (empty($this->data)) {
          $this->data = $this->Quizcat->read();
          $this->set('id',$id);
       }else{
       	       $data = array(
       	       		'id'=>$this->data['Quizcat']['id'],
       	                'category_name'=>$this->data['Quizcat']['category_name'],
       	       		'access_type'=>$this->data['Quizcat']['accessType']
       	       );
       	        if($this->Quizcat->save($data,$validate = true)){
...
function dailyquizlist($id=NULL){
	$this->layout = 'admin';
   	    // Check admin athontication //
		$this->checkadmin();
		$this->Quizquestion->find('all');
		$this->paginate = array(
                         'limit' => 50,
                         'page'=>1,
		                 'conditions'=>'status="1" and cat_id='.$id
    				 );
	     $data = $this->paginate('Quizquestion');
//             echo "<pre>";print_r($data);die;
         $this->set('data',$data);
         $this->set('id',$id);
}

function deletedailyquiz($conId=NULL,$baseId=NULL){
                 $this->Quizquestion->updateAll(array('status'=>'0'),array('id'=>$conId));
		 $this->redirect('dailyquizlist/'.$baseId);
}

function editquizquestion($id=NULL,$id2=NULL){
    $this->layout = 'admin';
    $list = $this->Quizquestion->find('all',array('conditions'=>'id='.$id));
    $this->set('list',$list);
    $this->set('id2',$id2);
    $arecord = $this->Quizanswer->find('all',array('fields'=>'id,answer_text,answer_discription,result','conditions'=>array('qid'=>$list[0]['Quizquestion']['question_id'])));
    $this->set('adata',$arecord);
    if(!empty($this->data)){
        $tempint =array();
        $tempint['id']=$id;
        $tempint['question_text']=$this->data['Quizquestion']['question_text'];
        $this->Quizquestion->save($tempint);

        for($i=0;$i<count($this->data['Quizanswer']);$i++){
            $k = $i+1;
            $k =array();
            $k['id']=$this->data['Quizanswer'][$i]['id'];
            $k['answer_text']=$this->data['Quizanswer'][$i]['answer_text'];
            if(isset($this->data['Quizanswer'][$i]['result'])){
            $k['result']=$this->data['Quizanswer'][$i]['result'];
            }else{
               $k['result']=0;
            }
            if(isset($this->data['Quizanswer'][$i]['result'])){
            $k['answer_discription']=$this->data['Quizanswer'][$i]['answer_discription'];
            }
            $this->Quizanswer->save($k);
        }
        $this->Session->setFlash('Quiz content edit successfully');
        $this->redirect('dailyquizlist/'.$id2);
