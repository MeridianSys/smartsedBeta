<?php

class ImagesController extends AppController {

	var $uses = array('User');

	var $helpers = array('Ajax','Session');
         var $components = array('Session','Images');
        var $layout = 'default';

function imagecrop() {

     $uid = $this->Session->read('User.id');
     $userImage = $this->User->find(array('User.id' => $uid),array('Userprofile.image'));
     //print_r($userImage);
     if (!isset($_SESSION['random_key']) || strlen($_SESSION['random_key'])==0){
        $_SESSION['random_key'] = strtotime(date('Y-m-d H:i:s')); //assign the timestamp to the session variable
	$_SESSION['user_file_ext']= "";
     }

// Only one of these image types should be allowed for upload
$allowed_image_types = array('image/pjpeg'=>"jpg",'image/jpeg'=>"jpg",'image/jpg'=>"jpg",'image/png'=>"png",'image/x-png'=>"png",'image/gif'=>"gif");
$allowed_image_ext = array_unique($allowed_image_types); // do not change this
$image_ext = "";	// initialise variable, do not change this.
foreach ($allowed_image_ext as $mime_type => $ext) {
    $image_ext.= strtoupper($ext)." ";
}

    $error = '';

    $this->Images->upload_dir = WWW_ROOT . 'files' . DS . 'upload_pic' . DS;
//Image Locations
    if($userImage['Userprofile']['image']!='') {
        $this->Images->thumb_image_name = $userImage['Userprofile']['image'];
        $uploadedfile_ext = substr($userImage['Userprofile']['image'], strrpos($userImage['Userprofile']['image'], '.') + 1);
        $this->Images->large_image_name = $this->Images->large_image_name.$uid.".".$uploadedfile_ext;
         $large_image_location = $this->Images->upload_dir.$this->Images->large_image_name;
         $thumb_image_location = $this->Images->upload_dir.$this->Images->thumb_image_name;
    }else {
         $this->Images->large_image_name = $this->Images->large_image_name.$uid.$_SESSION['user_file_ext'];
         $this->Images->thumb_image_name = $this->Images->thumb_image_name.$uid.$_SESSION['user_file_ext'];
         $large_image_location = $this->Images->upload_dir.$this->Images->large_image_name;
         $thumb_image_location = $this->Images->upload_dir.$this->Images->thumb_image_name;
    }
//Create the upload directory with the right permissions if it doesn't exist
if(!is_dir($this->Images->upload_dir)){
    mkdir($this->Images->upload_dir, 0777);
    chmod($this->Images->upload_dir, 0777);
}
//echo $large_image_location;
//Check to see if any images with the same names already exist
if (file_exists($large_image_location)){
    if(file_exists($thumb_image_location)){
        $thumb_photo_exists = '/smartsed/files/upload_pic/'.$this->Images->thumb_image_name;
        }else{
        $thumb_photo_exists = "";
    }

    $large_photo_exists = '/smartsed/files/upload_pic/'. $this->Images->large_image_name;
    $this->set('current_large_image_width', $this->Images->getWidth($large_image_location));
    $this->set('current_large_image_height', $this->Images->getHeight($large_image_location));
} else {
    $thumb_photo_exists = '';
    $large_photo_exists = '';
}

    $this->set('large_photo_exists', $large_photo_exists);
    $this->set('thumb_photo_exists', $thumb_photo_exists);
    $this->set('thumb_width', $this->Images->thumb_width);
    $this->set('thumb_height', $this->Images->thumb_height);

if(!empty($this->data)) {

    if(isset($this->data['Image']['image'])){
//Get the file information
        $userfile_name = $this->data['Image']['image']['name'];
        $userfile_tmp = $this->data['Image']['image']['tmp_name'];
        $userfile_size = $this->data['Image']['image']['size'];
        $userfile_type = $this->data['Image']['image']['type'];
        $filename = basename($this->data['Image']['image']['name']);
        $file_ext = substr($filename, strrpos($filename, '.') + 1);

        if(in_array($file_ext,$allowed_image_ext)) {
                //Only process if the file is a JPG and below the allowed limit
                if((!empty($this->data['Image']["image"])) && ($this->data['Image']['image']['error'] == 0)) {

		foreach ($allowed_image_types as $mime_type => $ext) {
			//loop through the specified image types and if they match the extension then break out
			//everything is ok so go and check file size
			if($file_ext==$ext && $userfile_type==$mime_type){
				$error = "";
				break;
			}else{
				$error = "Only <strong>".$image_ext."</strong> images accepted for upload<br />";
			}
		}
		//check if the file size is above the allowed limit
		if ($userfile_size > ($this->Images->max_file*1048576)) {
			$error.= "Images must be under ".$this->Images->max_file. " MB in size";
		}

                }else{
                        $error= "Select an image for upload";
                }

                //Everything is ok, so we can upload the image.
                if (strlen($error)==0){
                    if (isset($this->data['Image']['image']['name'])){

                    //this file could now has an unknown file extension (we hope it's one of the ones set above!)
                    if($userImage['Userprofile']['image']!='') {
                         $large_image_location = $large_image_location;
                         $thumb_image_location = $thumb_image_location;
                    } elseif($_SESSION['user_file_ext']!=''){
                        $large_image_location = $large_image_location;
                         $thumb_image_location = $thumb_image_location;
                    }else {
                    $large_image_location = $large_image_location.".".$file_ext;
                    $thumb_image_location = $thumb_image_location.".".$file_ext;
                    }
                    //put the file ext in the session so we know what file to look for once its uploaded
                    $_SESSION['user_file_ext']=".".$file_ext;
                    //echo "===".$large_image_location;
                    //die;
                    move_uploaded_file($userfile_tmp, $large_image_location);
                    chmod($large_image_location, 0777);

                    $width = $this->Images->getWidth($large_image_location);
                    $height = $this->Images->getHeight($large_image_location);
                    //Scale the image if it is greater than the width set above
                    if ($width > $this->Images->max_width){
                        $scale = $this->Images->max_width/$width;
                        $uploaded = $this->Images->resizeImage($large_image_location,$width,$height,$scale);
                    }else{
                        $scale = 1;
                        $uploaded = $this->Images->resizeImage($large_image_location,$width,$height,$scale);
                    }

                    //Delete the thumbnail file so the user can create a new one
                    if (file_exists($thumb_image_location))
                    {
                        unlink($thumb_image_location);
                    }
                 }


                //Refresh the page to show the new uploaded image
               $this->redirect(array('controller' => "images", 'action' => 'imagecrop'));
            }
        } else {
            $error= "Select a valid image for upload. The valid extensions are ". $image_ext;
        }
}

        if(isset($this->data['Image']['x1']))
        {
        //Get the new coordinates to crop the image.
            $x1 = $this->data['Image']["x1"];
            $y1 = $this->data['Image']["y1"];
            $x2 = $this->data['Image']["x2"];
            $y2 = $this->data['Image']["y2"];
            $w = $this->data['Image']["w"];
            $h = $this->data['Image']["h"];
            //Scale the image to the thumb_width set above
            $scale = $this->Images->thumb_width/$w;
            $cropped = $this->Images->resizeThumbnailImage($thumb_image_location, $large_image_location,$w,$h,$x1,$y1,$scale);
            $fileName = explode("/",$cropped);
            $uploadedFileName = $fileName[11];  /// 9 for ronak , 11 for godaddy
            $this->User->updateAll(array('Userprofile.image' => "'$uploadedFileName'"),array('User.id'=>$uid));
            $this->redirect(array('controller' => "users", 'action' => 'myaccount'));
        }
        $this->set('error', $error);
    }
}

    function deleteImage()
    {
        $this->Images->upload_dir = WWW_ROOT . 'files' . DS . 'upload_pic' . DS;

        $thumb_image_location = $this->Images->upload_dir.$this->Images->thumb_image_name;

        if (file_exists($thumb_image_location))
        {
            unlink($thumb_image_location);
        }
        $this->redirect(array('controller' => "images", 'action' => 'imagecrop'));
    }
}
?>