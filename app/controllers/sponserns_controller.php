...
function sponsernquestions($id=NULL){
	...
	$result = $this->Sponsernanswer->query("SELECT COUNT(id) FROM `sponsern_answers` GROUP BY qid ORDER BY ID ASC LIMIT 1");
		...
		$this->paginate = array(
                         'limit' => 25,
                         'page'=>1,
		                 'conditions'=>'softdelete="0" and sponsern_id='.$id
...
function newsponsernquestion($id=NULL){
	...
	$contentArray['id']='';
        ...
        $contentArray['access_type']=$this->data['Sponsern']['accessType'];// 0:for All Student  1:for Non Medical 2:for Medical
        ...
        $contentArray['weblink']=preg_replace('#^https?://#', '', $this->data['Sponsern']['weblink']);
...
function editsponsernquestion($id=NULL,$id2=NULL,$flg=NULL){
    ...
    $this->set('accessList',$accessList);
    if(!empty($this->data)){
        $weblink_path=preg_replace('#^https?://#', '', $this->data['Sponsern']['weblink']);
        $this->Sponserncontent->updateAll(array('Sponserncontent.content_name'=>'"'.$this->data['Sponsern']['content_name'].'"','Sponserncontent.contentprovider'=>'"'.$this->data['Sponsern']['contentprovider'].'"','Sponserncontent.contentprovider_status'=>'"'.$this->data['Sponsern']['contentprovider_status'].'"','Sponserncontent.weblink'=>'"'.$weblink_path.'"','Sponserncontent.weblink_status'=>'"'.$this->data['Sponsern']['weblink_status'].'"','Sponserncontent.st_dt'=>'"'.$this->data['Sponsern']['st_dt'].'"','Sponserncontent.end_dt'=>'"'.$this->data['Sponsern']['end_dt'].'"'),array('Sponserncontent.id =' => $id));
        $tempint =array();
        $tempint['id']=$id;
        $tempint['access_type']=$this->data['Sponsern']['accessType'];
        ...
        for($i=0;$i<count($this->params['form']);$i++){
            ...
            foreach($this->params['form']['q'.$k]['hint'] as $keyhint=>$valhint){
               $this->Sponsernquestion->updateAll(array('Sponsernquestion.hint'=>'"'.$valhint.'"'),array('Sponsernquestion.id =' => $keyhint));
            }
            foreach($this->params['form']['q'.$k]['explanation'] as $keyexplanation=>$valexplanation){
               $this->Sponsernquestion->updateAll(array('Sponsernquestion.explanation'=>'"'.$valexplanation.'"'),array('Sponsernquestion.id =' => $keyexplanation));
            }
...
function sponsorlist($id2=NULL,$id3=NULL){
    ...
    $resultOne = $this->Sponserncontent->find('all',array('conditions'=>'Sponserncontent.st_dt <= NOW() AND NOW() <= Sponserncontent.end_dt AND STATUS ="1"'.$addQuery));
    ...
    $resultTwo = $this->Sponsernquestion->find('all',array('conditions'=>'Sponsernquestion.id IN('.$refinepool.')'));
    for($m=0;$m<count($resultTwo);$m++){
            $newcont[]=$resultTwo[$m]['Sponsernquestion']['sponsern_content_id'];
            }
    $unicontid=implode(",", array_unique($newcont));

$resultThree = $this->Sponserncontent->find('all',array('conditions'=>'Sponserncontent.id IN('.$unicontid.')'));
for($n=0;$n<count($resultThree);$n++){
	$newcat[]=$resultThree[$n]['Sponserncontent']['sponsern_id'];
	}
 $unicatid=implode(",", array_unique($newcat));
   if($unicatid!=''){
    $resultFeatured = $this->Sponsern->find('all',array('conditions'=>'Sponsern.id IN('.$unicatid.') AND status = 1', 'order'=>'index DESC'));
    }else{
       $resultFeatured=array();
    }
...
function studentquiz($sponseredId=NULL,$skipId=NULL){
   ...
   $this->checkuserlog();
	 ...
	 * Content on the day 
	 */
	$resultOne = $this->Sponserncontent->find('all',array('conditions'=>'Sponserncontent.st_dt <= NOW() AND NOW() <= Sponserncontent.end_dt AND STATUS ="1" AND sponsern_id ='.$sponseredId.$addQuery));
	...
	if(!empty($questionpool)){
		...
		for($m=0;$m<count($resultcontent[0]['Sponsernquestion']);$m++){
                    $totqus[]=$resultcontent[0]['Sponsernquestion'][$m]['id'];
                }
		...
		$this->set('questionofday',$questionofday[0]);
            ...
            if($skipId!=''){
            $this->redirect('studentquiz/'.$sponseredId);
            }
        }
	if(!empty($this->data)){
		pr($this->params);
...
function copysponsern($copyId=NULL,$id=NULL){
        $contentArray['id']='';
        $contentArray['content_type']=$this->data['Sponsern']['content_type'];//1:for text 2:for Image  3:for Vedio
        $contentArray['content_name']=$this->data['Sponsern']['content_name'];
        $contentArray['instructions']=$this->data['Sponsern']['instructions'];
        ...
        if($this->data['Sponsern']['content_type']==1){
          $contentArray['text']=$this->data['Sponsern']['text'];	
        }elseif($this->data['Sponsern']['content_type']==3 || $this->data['Sponsern']['content_type']==2){
        	$imagevideo = $this->Sponserncontent->find('all',array('fields'=>'image_video','conditions'=>'id='.$copyId));
        	$file = $imagevideo[0]['Sponserncontent']['image_video'];
        	$ext = explode('.',$file);
        	$newfile = md5(date('Y-m-d H:i:s')).'.'.$ext[1];
            @chmod(_ROOT_BASE_PATH."img/sponsern/", 0777);
        	if (!copy(_ROOT_BASE_PATH."img/sponsern/".$file, _ROOT_BASE_PATH."img/sponsern/".$newfile)) {
    			//
			}else{
        ...
        $questionArray = array();
         for($i=0;$i<count($this->params['form']);$i++){
            ...
            foreach($this->params['form']['q'.$k]['question'] as $key=>$val){
             ...
             foreach($this->params['form']['q'.$k]['answer'] as $keyAns=>$valAns){
               $answerArray['id']='';
               $answerArray['qid'] = $this->Sponsernquestion->id;
       ...
       $this->redirect('editsponsernquestion/'.$stdContentId.'/'.$id);
        
}
function movetosponarchive($id=NULL,$base=NULL,$flg=NULL){
	if($flg!=''){
	$this->Sponserncontent->updateAll(array('softdelete'=>'0'),array('id'=>$id));
	$this->redirect('sponsernsarchive');	
	}else{
	$this->Sponserncontent->updateAll(array('softdelete'=>'1'),array('id'=>$id));
	$this->redirect('sponsernquestions/'.$base);
	}
}
function sponsernsarchive(){
	$this->layout = 'admin';
	$calender = $this->Sponserncontent->find('all');
	$answer = $this->Sponsernquestion->find('all');
	// For Total //
