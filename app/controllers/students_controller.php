...
function index(){
    ...
    if($str!=''){
    $this->paginate['Studentcontent']=array('conditions'=>'Studentcontent.softdelete="0" and Studentcontent.id not in ('.substr($str,1).') and Studentcontent.type="1"','page'=>1,'limit'=>25,'order'=>'Studentcontent.id DESC');	
    }else{
    $this->paginate['Studentcontent']=array('conditions'=>'Studentcontent.softdelete="0" and Studentcontent.type="1"','page'=>1,'limit'=>25,'order'=>'Studentcontent.id DESC');	
    }
...
function poll(){
    $this->checkadmin();
    $this->Studentcontent->find('all');
    $this->paginate['Studentcontent']=array('conditions'=>'Studentcontent.softdelete="0" and Studentcontent.type="2"','page'=>1,'limit'=>25,'order'=>'Studentcontent.id DESC');
    $data = $this->paginate('Studentcontent');
    $this->set('data',$data);
}
...
function studentcontent($id=NULL){
    ...
    if(!empty($this->data)){
        ...
        $contentArray['content_type']=$this->data['Students']['content_type'];//1:for text 2:for Image  3:for Video
        ...
        for($i=0;$i<count($this->params['form']);$i++){
            ...
            $questionArray['hint']=$this->params['form']['q'.($i+1)]['hint'][0];
            $questionArray['explanation']=$this->params['form']['q'.($i+1)]['explanation'][0];
...
function editcontent($id=NULL,$flg=NULL){
    ...
    if(!empty($this->data)){
        $weblink_path = preg_replace('#^https?://#', '', $this->data['Students']['weblink']);
        if($this->data['Students']['category_type']=='1'){
            $contentCatId=$this->data['Students']['categoryId'];
        }else{
            $contentCatArray['id']='';
        ...
        $this->Studentcontent->updateAll(array('Studentcontent.name'=>'"'.$this->data['Students']['name'].'"','Studentcontent.contentprovider'=>'"'.$this->data['Students']['contentprovider'].'"','Studentcontent.contentprovider_status'=>'"'.$this->data['Students']['contentprovider_status'].'"','Studentcontent.weblink'=>'"'.$weblink_path.'"','Studentcontent.weblink_status'=>'"'.$this->data['Students']['weblink_status'].'"','Studentcontent.category'=>'"'.$contentCatId.'"','Studentcontent.st_dt'=>'"'.$this->data['Students']['st_dt'].'"','Studentcontent.end_dt'=>'"'.$this->data['Students']['end_dt'].'"'),array('Studentcontent.id =' => $id));
        $tempint =array();
        $tempint['id']=$id;
        $tempint['access_type']=$this->data['Students']['accessType'];// 0:for All Student  1:for Non Medical 2:for Medical
        ...
        for($i=0;$i<count($this->params['form']);$i++){
            ...
            foreach($this->params['form']['q'.$k]['hint'] as $keyhint=>$valhint){
               $this->Studentquestion->updateAll(array('Studentquestion.hint'=>'"'.$valhint.'"'),array('Studentquestion.id =' => $keyhint));
            }
            foreach($this->params['form']['q'.$k]['explanation'] as $keyexplanation=>$valexplanation){
               $this->Studentquestion->updateAll(array('Studentquestion.explanation'=>'"'.$valexplanation.'"'),array('Studentquestion.id =' => $keyexplanation));
            }
...
* _______________________________________________________________________________
*/
function studentquiz($cid=NULL,$skipId=NULL){
	 ...
	 */
        ...
        if($userTypeAccess!='Dr'){
           $addQuery=' AND access_type !=2 AND category='.$cid;
        }else{
            $addQuery=' AND access_type !=1 AND category='.$cid;
        }
	 ...
	 * Content on the day 
	 */
	$resultOne = $this->Studentcontent->find('all',array('conditions'=>'Studentcontent.st_dt <= NOW() AND NOW() <= Studentcontent.end_dt AND STATUS ="1" AND type =1'.$addQuery));
	...
	$questionpoolmain = array_values(array_diff($newar, $questionplayed));
	if($skipId!=''){
		...
		for($m=0;$m<count($resultcontent[0]['Studentquestion']);$m++){
                    $totqus[]=$resultcontent[0]['Studentquestion'][$m]['id'];
                }
                $remquestionpool = array_values(array_intersect($questionpool, $totqus));
                $totquestionpool = count(array_values(array_intersect($questionpoolmain, $totqus)));
                $currentquestionnum=$totquestionpool-count($remquestionpool)+1;
                $this->set('totquestionpool',$totquestionpool);
                $this->set('currentquestionnum',$currentquestionnum);
                $this->set('data',$resultcontent[0]['Studentcontent']);
		$this->set('questionofday',$questionofday[0]);
		
	}else{
            if($skipId!=''){
            $this->redirect('studentquiz/'.$cid);
            }
        }
	if(!empty($this->data)){
				...
				if($skipId!=''){
				$this->redirect('quizresult/'.$cid.'/'.$qa[0].'/'.$skipId);
				}else{
					$this->redirect('quizresult/'.$cid.'/'.$qa[0]);
				}
...
* _______________________________________________________________________________
*/
function quizresult($cid=NULL, $qid=NULL,$skipId=NULL){
    $this->checkuserlog();
    $this->layout='default';
    $this->set('cid',$cid);
    $userID=$this->Session->read('User.id');
...
function studentpoll($skipId=NULL){
    ...
    $userTypeAccess=substr($userType['Userprofile']['title'], 0, -1);
        if($userTypeAccess!='Dr'){
           $addQuery=' AND access_type !=2';
        }else{
            $addQuery=' AND access_type !=1';
        }
        /*
	 * Student played quiz with content ID 
	 */
	 ...
	 * Content on the day 
	 */
	$resultOne = $this->Studentcontent->find('all',array('conditions'=>'Studentcontent.st_dt <= NOW() AND NOW() <= Studentcontent.end_dt AND STATUS ="1" AND type =2'.$addQuery));
...
function cafeseries(){
    ...
    $this->paginate['Cafeseriescontent']=array('conditions'=>'softdelete = 0','page'=>1,'limit'=>25);
...
function videoeditcontent($id=NULL,$flg=NULL){
    ...
    if(!empty($this->data)){
        ...
        for($i=0;$i<count($this->params['form']);$i++){
            ...
            foreach($this->params['form']['q'.$k]['hint'] as $keyhint=>$valhint){
               $this->Cafeseriesquestion->updateAll(array('Cafeseriesquestion.hint'=>'"'.$valhint.'"'),array('Cafeseriesquestion.id =' => $keyhint));
             }
            foreach($this->params['form']['q'.$k]['explanation'] as $keyexplanation=>$valexplanation){
               $this->Cafeseriesquestion->updateAll(array('Cafeseriesquestion.explanation'=>'"'.$valexplanation.'"'),array('Cafeseriesquestion.id =' => $keyexplanation));
             }
            foreach($this->params['form']['q'.$k]['answer'] as $keyAns=>$valAns){
...
function usercafeseries($skipId=NULL){
   ...
   $this->checkuserlog();
	 ...
	 * Content on the day 
	 */
	$resultOne = $this->Cafeseriescontent->find('all',array('conditions'=>'Cafeseriescontent.st_dt <= NOW() AND NOW() <= Cafeseriescontent.end_dt AND STATUS ="1"'.$addQuery));
	//pr($resultOne);
	for($i=0;$i<count($resultOne);$i++){
	...
	if(!empty($questionpool)){
		...
		for($m=0;$m<count($resultcontent[0]['Cafeseriesquestion']);$m++){
                    $totqus[]=$resultcontent[0]['Cafeseriesquestion'][$m]['id'];
                }
                $remquestionpool = array_values(array_intersect($questionpool, $totqus));
                $totquestionpool = count(array_values(array_intersect($questionpoolmain, $totqus)));
                $currentquestionnum=$totquestionpool-count($remquestionpool)+1;
...
$totearnpts = $this->ediv_user_masters->find('all',array('conditions'=>'uid='.$userID.' and ediv_type=8 and date_format(datetime, "%Y-%m-%d") = "'.$dt.'"'));
if(count($resultQuizbtn)>0){
    $resultDataurl = $this->Quizuserattempt->query("SELECT date_format(played_date, '%Y-%m-%d %H:%i') AS PLDT FROM `quiz_user_attempts` where uid =".$this->Session->read('User.id')." and date_format(played_date, '%Y-%m-%d')='".$dt."' GROUP BY date_format(played_date, '%Y-%m-%d %H:%i')");
 	$this->set('hide','hide');
	$this->set('totearnpt',round($totearnpts[0]['ediv_user_masters']['points']));
	$this->set('timetook',$resultQuizbtn[0]['Quizuserattempt']['timetook']);
	$this->set('reviewlink',$resultDataurl[0][0]['PLDT']);
}
$resultData = $this->Quizuserattempt->query("SELECT DATE_FORMAT(played_date, '%m/%d/%Y') AS DT, date_format(played_date, '%Y-%m-%d %H:%i') AS PD, GROUP_CONCAT(DISTINCT(catid) SEPARATOR ',') AS catid , SUM(points) AS point FROM `quiz_user_attempts` where uid =".$this->Session->read('User.id')." and date_format(played_date, '%Y-%m-%d')!='".$dt."' GROUP BY date_format(played_date, '%Y-%m-%d %H:%i') ORDER BY played_date DESC ");
...
$resultContentData = $this->Studentuserattempt->query("SELECT DATE_FORMAT(played_date, '%m/%d/%Y') AS DT, date_format(played_date, '%Y-%m-%d %H:%i') AS PD, content_id, qid, SUM(points) AS point FROM `student_user_attempts` where uid =".$this->Session->read('User.id')." AND type=1  GROUP BY content_id ORDER BY played_date DESC ");
	...
	for($i=0;$i<count($resultContentData);$i++){
            ...
            $studyData[$i]['contentLink']=_HTTP_PATH.'students/archivequizresult/'.$qid.'/'.$contentId;
...
$resultConceptData = $this->Studentuserattempt->query("SELECT count(id) AS tot, DATE_FORMAT(played_date, '%m/%d/%Y') AS DT, date_format(played_date, '%Y-%m-%d %H:%i') AS PD, content_id, qid, SUM(points) AS point FROM `student_user_attempts` where uid =".$this->Session->read('User.id')." AND type=2  GROUP BY content_id ORDER BY played_date DESC ");
	...
	for($i=0;$i<count($resultConceptData);$i++){
            ...
            $conceptData[$i]['conceptLink']=_HTTP_PATH.'students/archivepollresult/'.$qid.'/'.$conceptId;
//            $conceptData[$i]['catName']=$conceptDetails[0]['student_contents']['name'].' ('.$conceptDetails[0]['student_contents']['category'].'-'.$conceptType.')';
            ...
            $conceptData[$i]['point']=(strlen($resultConceptData[$i][0]['point'])==5)? '0'.$resultConceptData[$i][0]['point']:$resultConceptData[$i][0]['point'];
...
$resultCafeData = $this->Cafeseriesuserattempt->query("SELECT DATE_FORMAT(played_date, '%m/%d/%Y') AS DT, date_format(played_date, '%Y-%m-%d %H:%i') AS PD, qid, cafeseries_id, SUM(points) AS point FROM `cafeseries_user_attempts` where uid =".$this->Session->read('User.id')." GROUP BY cafeseries_id ORDER BY played_date DESC ");
	...
	for($i=0;$i<count($resultCafeData);$i++){
            ...
            $cafeData[$i]['cafeLink']=_HTTP_PATH.'students/archivevideoresult/'.$qid.'/'.$cafeId;
...
$resultSponsoredData = $this->Sponsernuserattempt->query("SELECT DATE_FORMAT(played_date, '%m/%d/%Y') AS DT, date_format(played_date, '%Y-%m-%d %H:%i') AS PD, sponsern_content_id, qid, sponsern_id, SUM(points) AS point FROM `sponsern_user_attempts` where uid =".$this->Session->read('User.id')." GROUP BY sponsern_content_id ORDER BY played_date DESC ");
	...
	for($i=0;$i<count($resultSponsoredData);$i++){
            ...
            $sponsoredData[$i]['sponsoredLink']=_HTTP_PATH.'students/archivesponsorresult/'.$qid.'/'.$sponsernId.'/'.$sponsernContentId;
...
function copystudentcontent($id=NULL){
	$contentArray['id']='';
        $contentArray['type']=$this->data['Students']['type'];// 1:for Student Quiz  2:for Student Poll
        $contentArray['category']=$this->data['Students']['categoryId'];
        $contentArray['name']=$this->data['Students']['name'];
        $contentArray['instructions']=$this->data['Students']['instructions'];
        $contentArray['strategy']=$this->data['Students']['strategy'];
        $contentArray['script']=$this->data['Students']['script'];
...
//// archive to cafe student pol sponsern start ///
function movetoarchive($id=NULL,$flg=NULL){
	if($flg!=''){
	$this->Cafeseriescontent->updateAll(array('softdelete'=>'0'),array('id'=>$id));
	$this->redirect('cafearchive');	
	}else{
	$this->Cafeseriescontent->updateAll(array('softdelete'=>'1'),array('id'=>$id));
	$this->redirect('cafeseries');
	}
}
function cafearchive(){
	$this->checkadmin();
    $this->Cafeseriescontent->find('all');
    $this->paginate['Cafeseriescontent']=array('conditions'=>'softdelete = 1','page'=>1,'limit'=>25);
    $data = $this->paginate('Cafeseriescontent');
    $this->set('data',$data);
}
function movetoarchivepoll($id=NULL,$flg=NULL){
	if($flg!=''){
	$this->Studentcontent->updateAll(array('softdelete'=>'0'),array('id'=>$id));
	$this->redirect('todayarchive');	
	}else{
	$this->Studentcontent->updateAll(array('softdelete'=>'1'),array('id'=>$id));
	$this->redirect('poll');
	}
}
function todayarchive(){
	$this->checkadmin();
    $this->Studentcontent->find('all');
    $this->paginate['Studentcontent']=array('conditions'=>'Studentcontent.softdelete="1" and Studentcontent.type="2"','page'=>1,'limit'=>25,'order'=>'Studentcontent.category');
    $data = $this->paginate('Studentcontent');
    $this->set('data',$data);
}
function movetoarchivestudent($id=NULL,$flg=NULL){
	if($flg!=''){
	$this->Studentcontent->updateAll(array('softdelete'=>'0'),array('id'=>$id));
	$this->redirect('contentarchive');	
	}else{
	$this->Studentcontent->updateAll(array('softdelete'=>'1'),array('id'=>$id));
	$this->redirect('index');
	}
...
function contentarchive(){
	$this->checkadmin();
    $this->Studentcontent->find('all');
    $this->paginate['Studentcontent']=array('conditions'=>'Studentcontent.softdelete="1" and Studentcontent.type="1"','page'=>1,'limit'=>25,'order'=>'Studentcontent.category');
    $data = $this->paginate('Studentcontent');
    $this->set('data',$data);
}
...
$questionpool = array_values(array_diff($newar, $questionplayed));
$refinepool = implode(",", $questionpool);

$resultTwo = $this->Studentquestion->find('all',array('conditions'=>'Studentquestion.id IN('.$refinepool.')'));
for($m=0;$m<count($resultTwo);$m++){
	$newcont[]=$resultTwo[$m]['Studentquestion']['content_id'];
	}
$unicontid=implode(",", array_unique($newcont));

$resultThree = $this->Studentcontent->find('all',array('conditions'=>'Studentcontent.id IN('.$unicontid.')'));
for($n=0;$n<count($resultThree);$n++){
	$newcat[]=$resultThree[$n]['Studentcontent']['category'];
	}
 $unicatid=implode(",", array_unique($newcat));
   if($unicatid!=''){
    $result = $this->Studentcontentcat->find('all',array('conditions'=>'Studentcontentcat.id IN('.$unicatid.')'));
    }else{
       $result=array();
    }
