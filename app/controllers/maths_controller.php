...
function enteranswer($id=NULL,$flg=NULL){
	 	$this->checkadmin();
	 	$resultCounter = $this->Studentanswer->find('all',array('conditions'=>'qid='.$id));
	 	if(count($resultCounter)>0){
	 		$num = count($resultCounter)+1;
	 	}else{
	 		$num = 1;
	 	}
	 	$this->set('num',$num);
...
* Parameters : $id=NULL,$option=NULL and they hold the value when required
*
*_____________________________________________________________
*/
