    ...
    if(!empty($this->data)) {
        if(isset($this->data['Userprofile']['final']) && $this->data['Userprofile']['final']=='final') {
           ...
           if($this->data['Userprofile']['adduce']!='') {
		    ...
		    // 2 : Doctor(2,0) 1: Non Doctor(1,0) 0:General //
           
           if($this->data['Userprofile']['title']=="Dr."){
           	$this->data['Userprofile']['quiz_cat']=$this->getdrnondrcatid(2); 
           }else{
           	$this->data['Userprofile']['quiz_cat']=$this->getdrnondrcatid(1);
           }
...
function dashboard($acorid=NULL,$id2=NULL,$id3=NULL) {
 ...
 if(count($countchallenge)>0){
     $this->redirect(array('controller'=>'challenges','action'=>'challengedto'));
 }
/*/////////////////////////////////////////////////////////
  * ********************************************************
...
$toeflQuery = $this->Quizuserattempt->find('all', array(
                                                        ...
                                                        'limit' => 5));
$edivEarnQuery= array();
if(count($toeflQuery)>0) {
        for($i=0; $i<count($toeflQuery); $i++) {
            $totwin=$this->Quizuserattempt->query('SELECT count(id) AS totwin  FROM `quiz_user_attempts` WHERE `uid` = '.$toeflQuery[$i]['Quizuserattempt']['uid'].' AND `final_result` = 1 AND `catid` = '.$toeflQuery[$i]['Quizuserattempt']['catid']);
            $totatmpt=$this->Quizuserattempt->query('SELECT count(id) AS totatmpt  FROM `quiz_user_attempts` WHERE `uid` = '.$toeflQuery[$i]['Quizuserattempt']['uid'].' AND `catid` = '.$toeflQuery[$i]['Quizuserattempt']['catid']);
//            $totquiz=$this->quiz_question_masters->query('SELECT count(id) AS totqus  FROM `quiz_question_masters` WHERE `cat_id` = '.$toeflQuery[$i]['Quizuserattempt']['catid'].' AND `status` = 1');
//            $edivEarn_totpts = $totquiz[0][0]['totqus']*$currentVal;
            $currentVal=Configure::read('contoller')->pointforediv(8);
            $edivEarn_totwin = $totwin[0][0]['totwin']*$currentVal;
            $edivEarn_totatmptpts = $totatmpt[0][0]['totatmpt']*$currentVal;
//            $edivEarnQuery[$i][0]['Total'] = round($edivEarn_totwin*100/$edivEarn_totpts);
            $edivEarnQuery[$i][0]['Total'] = round($edivEarn_totwin*100/$edivEarn_totatmptpts);
            $edivEarnQuery[$i]['Quizuserattempt']['catid'] = $toeflQuery[$i]['Quizuserattempt']['catid'];
            $edivEarnQuery[$i]['Quizuserattempt']['uid'] = $toeflQuery[$i]['Quizuserattempt']['uid'];
        }
}

//$this->set('resToefl',$toeflQuery);
$this->set('resToefl',$edivEarnQuery);
...
function myaccount($val=NULL,$editVal=NULL){
...
$this->data = $this->User->read();
 ...
 // Ediv Tab //
  $edivUser = $this->Ediv_user_master->find('all',array('conditions'=>array('uid' => $userID,  'MONTH(datetime)' => date('m')),'order'=>array('id DESC')));
...
$multiplier = $this->Edivcharityvalue->find('all',array('fields'=>'ediv_charity_amount','conditions'=>'Edivcharityvalue.status=1','limit'=>1));
  ...
  $userCollection = $this->Charityuser->find('all',array('fields'=>"(SUM(Quizuserattempt.final_result))*(".($multiplier[0]['Edivcharityvalue']['ediv_charity_amount']/100).") as Collect,Charityuser.target",'conditions' =>'Charityuser.uid='.$userID.' And Charityuser.charity_id = '.$usercharity[0]['Charityuser']['charity_id'],'group' => 'Charityuser.uid'));
  ...
  // For target question //
  // Price Set
  $dateRe = $this->Charityuser->query("SELECT DATE_FORMAT(DATE_ADD('".$usercharity[0]['Charityuser']['start_date']."', INTERVAL 3 MONTH), '%Y-%m-%d') as dt from charity_users where uid =".$userID." ");
  $finalDt = $dateRe[0][0]['dt'];
  $priceSet = $multiplier[0]['Edivcharityvalue']['ediv_charity_amount'];
  $this->set('finalDt',$finalDt);
  $this->set('priceSet',$priceSet);
  $this->set('sumUser',$sum);
  //echo $communityFriendsUserCollection[0]['Charityuserfriend']['amount'];
  $this->set('familyUserAmount',$communityFriendsUserCollection[0]['Charityuserfriend']['amount']);
...
$userCountry = $this->countryname($this->data['Userprofile']['country']);
if($this->data['Userprofile']['country']!=40){
	    		$this->set('userState',$this->data['Userprofile']['otherstate']);
                        $this->set('userCity',$this->data['Userprofile']['othercity']);

    		}else{
	    		$this->set('userState',$this->statename($this->data['Userprofile']['state']));
	    		$this->set('userCity',$this->cityname($this->data['Userprofile']['city']));

    	}
if($this->data['Userprofile']['hobby']!=''){
            $hobby = explode(",",$this->data['Userprofile']['hobby']);
...
if($this->data['Userprofile']['institute']!=''){
            $resultInstitute = $this->Institution_list->find('all',array('conditions'=>'id ='.$this->data['Userprofile']['institute']));
            $institutionName = $resultInstitute[0]['Institution_list']['name'];
        }
if($this->data['Userprofile']['professional_track']!=''){
    $resultProfessional = $this->users_professional_tracks->find('all',array('conditions'=>'id ='.$this->data['Userprofile']['professional_track']));
    $professionalName = $resultProfessional[0]['users_professional_tracks']['professional_track'];
}

if($this->data['Userprofile']['passing_year']!='0'){
 $passing_year = $this->data['Userprofile']['passing_year'];
}

$lanuageNative = $this->languagename($this->data['Userprofile']['nativelanguage']);
$lanuageFluent = $this->languagename($this->data['Userprofile']['fluentlanguage']);
$lanuageFluent2 = $this->languagename($this->data['Userprofile']['fluentlanguage2']);
$lanuageFluent3 = $this->languagename($this->data['Userprofile']['fluentlanguage3']);
$lanuageLearn = $this->languagename($this->data['Userprofile']['learnlanguage']);
$lanuageLearn2 = $this->languagename($this->data['Userprofile']['learnlanguage2']);
$lanuageLearn3 = $this->languagename($this->data['Userprofile']['learnlanguage3']);
...
$this->set('eEditFlg',$eEditFlg);
$this->set('proEditFlg',$proEditFlg);
$this->set('acEditFlg',$acEditFlg);
$this->set('lanEditFlg',$lanEditFlg);
    ...
    $this->autoRender=false;
          if($this->RequestHandler->isAjax()){
             Configure::write('debug', 0);
                 if(!empty($_POST)){
                   $sum=0;
                    $edivUser = $this->Ediv_user_master->query('SELECT * FROM ediv_user_masters WHERE uid ='.$userID.' AND  MONTH(datetime) ='. $_POST['month'].' ORDER BY id DESC');
                   ...
                   $str.='<tr>
                     ...
                     <td width="48%" align="left" valign="middle" >';
                        if($edivUser[$ii]['ediv_user_masters']['ediv_type']==1){ $str.='Referral';}
                        if($edivUser[$ii]['ediv_user_masters']['ediv_type']==3){ $str.= 'Polls Surveys';}
                        if($edivUser[$ii]['ediv_user_masters']['ediv_type']==4){ $str.= 'Language Lab';}
                        if($edivUser[$ii]['ediv_user_masters']['ediv_type']==6){ $str.= 'Registration';}
                        if($edivUser[$ii]['ediv_user_masters']['ediv_type']==7){ $str.= 'eDiv Reward';}
                        if($edivUser[$ii]['ediv_user_masters']['ediv_type']==8){ $str.= 'Daily Quiz';}
                        if($edivUser[$ii]['ediv_user_masters']['ediv_type']==9){ $str.= 'Sponsored Point';}
                        if($edivUser[$ii]['ediv_user_masters']['ediv_type']==10){ $str.= 'Student Content';}
                        if($edivUser[$ii]['ediv_user_masters']['ediv_type']==11){ $str.= 'Student Content';}
                        if($edivUser[$ii]['ediv_user_masters']['ediv_type']==12){ $str.= 'Cafe Series';}
                    $str .='</td>
                   ...
                   $str .='<tr>
                       <td colspan="3" align="center" style="color:#000000;"><strong>'.__d("account","no-activity",true).'</strong></td>
                   </tr>';
                   }
                   $str .='</table>';
                     echo $str.'_'.$sum;
...
/// Static functions made //
    ...
    // Partner
     function partner(){
    	$this->layout='default';
        $data = $this->Staticcontent->find('all',array('conditions'=>'id = "3" and status=1 '));
        $partnerdata = $this->Static_partner->find('all',array('conditions'=>'status=1','order' =>  array('index ASC')));
        $this->set('data',$data);
        $this->set('partnerdata',$partnerdata);
    }
    // term
     function terms(){
    	$this->layout='default';
    ...
    $this->autoRender=false;
          if($this->RequestHandler->isAjax()){
             Configure::write('debug', 0);
                ...
                if($this->data['Userprofile']['editType']=='proEdit'){
                ...
                if($this->data['Userprofile']['editType']=='acEdit'){
                    $title = $this->data['Userprofile']['title'];
                    if($this->data['Userprofile']['title']=='Dr.'){
                ...
                if($this->data['Userprofile']['editType']=='lanEdit'){
                $ret = $this->User->updateAll(
                                              array('Userprofile.nativelanguage'=>'"'. $this->data['Userprofile']['nativelanguage'],'"',
...
function processpayment($uid=NULL,$register=NULL){
	        ...
	        if(count($user)>0){
	        $chkpayment = $this->get_user_info($user[0]['User']['id'], 'Userprofile.payment_status',false);
           // $($chkpayment['Userprofile']['payment_status']=='1'){	
	        $payment = array();
			$this->set('uid',$uid);
			$this->set('user',$user[0]);
			if($chkpayment['Userprofile']['payment_status']=='1'){
				 $this->Session->setFlash('Please login here');
				$this->redirect('/users/index');
			}
			elseif($this->checkeduemail($user[0]['User']['email'])==1){
			 $curdatetime = date('Y-m-d H:i:s');
   					...
   					if($response[1]==1){// Success fully enter in the site
						...
						$dashboardStatus = $this->get_user_info($user[0]['User']['id'], 'Userprofile.edivactivity_reminder',false);
			               if($dashboardStatus['Userprofile']['edivactivity_reminder']=='P' || $dashboardStatus['Userprofile']['edivactivity_reminder']=='S' || $dashboardStatus['Userprofile']['edivactivity_reminder']=='N' || $dashboardStatus['Userprofile']['edivactivity_reminder']=='') {
			                ...
			                $payment['plan_type'] =$this->data['User']['plan_type'];
	        	...
	        	if($register!=''){
	        		$this->set('failureresponse','Please make your payment to continue in site');
	        	}
			}
	       }else{
	       
	       	$this->set('failureresponse','You are not the existing user please make registration with us <a href="'._HTTP_PATH.'users/register">click here</a>');
	       }
...
function charityfeedback(){
$this->autoRender=false;
   if($this->RequestHandler->isAjax()){
    Configure::write('debug', 0);
	$temp['id']='';
   	$temp['uid']=$this->data['Charityfeedback']['uid'];
   	$temp['feedback']=$this->data['Charityfeedback']['feedback'];
   	$chrId = $this->Charityuser->find('all',array('fields'=>'charity_id','conditions'=>'uid ='.$this->data['Charityfeedback']['uid']));
   	$temp['charity_id']=$chrId[0]['Charityuser']['charity_id'];
    $this->Charityfeedback->save($temp);   	 
   }
}
function morecharityfriends(){
	$this->autoRender=false;
    if($this->RequestHandler->isAjax()){
    Configure::write('debug', 0);
    $friendemail = explode(',',$this->data['Users']['userfriendfamily']);
    $userID=$this->Session->Read('User.id'); 
    	 	$name = $this->UserFirstName($userID);
    	 	$usercharityid = $this->Charityuser->find('all',array('conditions'=>'uid='.$userID));
    	 	...
    	 	for($i=0;$i<count($friendemail);$i++){
				if($this->isValidEmail($friendemail[$i])){ // after validation send email to this email - id
					$friendarray= array();	
					$friendarray['id']='';
					$friendarray['uid']=$userID;
					$friendarray['user_charity_id']=$usercharityid[0]['Charityuser']['id'];
					$friendarray['friend_email']=$friendemail[$i];
					$friendarray['acknowledge']=1;
					$friendarray['charity_id'] = $usercharityid[0]['Charityuser']['charity_id'];
...
* ___________________________________________________________________________________________________
*/	   
function viewusercharity(){
	
	$this->Charityuserfriend->bindModel(array(
    'hasOne' => array(
        'Charityuser' => array(
            'foreignKey' => false,
            'conditions' => array('Charityuserfriend.user_charity_id = Charityuser.id')
        )
      )
   ));
	$result = $this->Charityuserfriend->find('all',array('conditions'=>'Charityuserfriend.uid='.$this->Session->Read('User.id')));
	
	$this->set('result',$result);
	
}
function userchangegoal(){
	$this->autoRender=false;
    if($this->RequestHandler->isAjax()){
    Configure::write('debug', 0);
    $userID=$this->Session->Read('User.id'); 
    $dt = $this->Charityuser->find('all',array('fields'=>'start_date','conditions'=>'uid='.$userID));
    
    	$result = $this->Charityuser->query("update charity_users set target = ".$this->data['Charityuser']['target'].",start_date='".$dt[0]['Charityuser']['start_date']."' where uid = ".$userID." ");
    }
