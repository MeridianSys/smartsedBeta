...
class StudentsController extends AppController {
	...
	var $name = 'Students';
        var $uses = array('Studentcontent','Studentquestion','Studentanswer','Studentuserattempt','ediv_user_masters','Cafeseriescontent','Cafeseriesquestion','Cafeseriesanswer','Cafeseriesuserattempt','Pausetempuser','Sponsern','Sponserncontent','Sponsernuserattempt','Sponsernquestion','Sponsernanswer');
...
function editcontent($id=NULL,$flg=NULL){
    ...
    if(!empty($this->data)){
        $this->Studentcontent->updateAll(array('Studentcontent.name'=>'"'.$this->data['Students']['name'].'"','Studentcontent.instructions'=>'"'.$this->data['Students']['instructions'].'"','Studentcontent.strategy'=>'"'.$this->data['Students']['strategy'].'"','Studentcontent.script'=>'"'.$this->data['Students']['script'].'"','Studentcontent.category'=>'"'.$this->data['Students']['category'].'"','Studentcontent.st_dt'=>'"'.$this->data['Students']['st_dt'].'"','Studentcontent.end_dt'=>'"'.$this->data['Students']['end_dt'].'"'),array('Studentcontent.id =' => $id));
        if($this->data['Students']['content_type']==1){
...
* _______________________________________________________________________________
*/
function editimagevideo($id=NULL){
   $this->checkadmin();
...
function videoeditcontent($id=NULL,$flg=NULL){
    ...
    if(!empty($this->data)){
        $this->Cafeseriescontent->updateAll(array('Cafeseriescontent.name'=>'"'.$this->data['Students']['name'].'"','Cafeseriescontent.instructions'=>'"'.$this->data['Students']['instructions'].'"','Cafeseriescontent.strategy'=>'"'.$this->data['Students']['strategy'].'"','Cafeseriescontent.script'=>'"'.$this->data['Students']['script'].'"','Cafeseriescontent.st_dt'=>'"'.$this->data['Students']['st_dt'].'"','Cafeseriescontent.end_dt'=>'"'.$this->data['Students']['end_dt'].'"'),array('Cafeseriescontent.id =' => $id));
   ...
   if(!empty($this->data)){
         ...
         if($ext[1]=='mp3' || $ext[1]=='x-flv' || $ext[1]=='flv' || $ext[1]=='mp4' || $ext[1]=='mpg' || $ext[1]=='wav'){
    			$rval = $this->Usercomponent->imageGenerateKey();
           	    	...
           	    	$this->Session->setFlash('Video updated successfully');
    			$this->redirect('videoeditcontent/'.$id);
           	    }
                     }else{
                         $this->Session->setFlash('Video size should not more than 5MB');
    			$this->redirect('videoeditcontent/'.$id);
...
* _______________________________________________________________________________
*/
function removequestion($conId=NULL){
	
	$result = $this->Cafeseriesquestion->find('all',array('fields'=>'MAX(id) as id','conditions'=>'cafeseries_id='.$conId, 'group'=>'cafeseries_id'));	
	$this->Cafeseriesanswer->deleteAll(array('Cafeseriesanswer.qid'=>$result[0][0]['id']));//$result[0][0]['id'],$cascade = true);
	$this->Cafeseriesquestion->delete($result[0][0]['id']);
	$this->redirect('videoeditcontent/'.$conId);
}
...
* _______________________________________________________________________________
*/
function addanswercon($conId=NULL){
...
* _______________________________________________________________________________
*/
function removequestioncon($conId=NULL){
	
	$result = $this->Studentquestion->find('all',array('fields'=>'MAX(id) as id','conditions'=>'content_id='.$conId, 'group'=>'content_id'));	
	$this->Studentanswer->deleteAll(array('Studentanswer.qid'=>$result[0][0]['id']));//$result[0][0]['id'],$cascade = true);
	$this->Studentquestion->delete($result[0][0]['id']);
	$this->redirect('editcontent/'.$conId);
}
