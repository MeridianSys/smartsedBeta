...
(function($) {
	...
	$.anythingSlider = function(el, options) {
		...
		base.init = function(){
			...
			base.regex = new RegExp('panell' + base.runTimes + '-(\\d+)', 'i'); // hash tag regex
                        //alert(base.regex);
		...
		base.updateSlider = function(){
			...
			base.currentPage = base.currentPage || 3;
			...
			// We just added two items, time to re-cache the list, then get the dimensions of each panel
                        base.$items = base.$el.children().addClass('panell' + (o.vertical ? ' vertical' : ''));
                        base.setDimensions();
			...
			if (o.buildNavigation && (base.pages > 1)) {
				...
				base.$items.filter(':not(.cloned)').each(function(j){
					...
					c = o.tooltipClass + ((i === 1) ? ' first' : '') + ((i === base.pages) ? ' last' : '');
                                        a = '<a class="panell' + i + '" href="#"><span>@</span></a>';
		...
		base.goForward2 = function() {
			base.gotoPage(base.currentPage + 5 * (o.playRtl ? -1 : 1), true);
		};
		...
		base.gotoPage = function(page, autoplay, callback, time) {
			...
			if (/^[#|.]/.test(page) && $(page).length) {
				page = $(page).closest('.panell').index() + base.adj;
			}
		...
		base.endAnimation = function(page, callback, time){
			...
			base.$items.removeClass('activePage').eq(2+page).addClass('activePage');
		...
		//	base.currentPage = page;
			...
			base.$currentPage = base.$items.removeClass('activePage').eq(2+page).addClass('activePage');
		...
		base.gotoHash = function(){
			...
			if (n === null && !/^#&/.test(h) && !/#!?\//.test(h)) {
				...
				n = ($(h).length && $(h).closest('.anythingBase')[0] === base.el) ? $(h).closest('.panell').index() : null;
			} else if (n !== null) {
				// #&panel1-3&panel3-3
